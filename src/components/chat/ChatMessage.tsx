...
const ChatMessage: React.FC<ChatMessageProps> = ({ message, formatMatchData }) => {
  switch (message.type) {
    ...
    case "ai-analysis":
      ...
      // Handle ai-analysis messages with playerStats
      if (message.playerStats) {
        return (
          <div className="flex justify-start mb-3">
            <Card className="w-full max-w-[95%] border-l-4 border-l-green-500 shadow-md">
              <CardContent className="p-4">
                ...
                <div className="space-y-2">
                  {message.playerStats.map((player, index) => (
                    <div key={index} className="bg-green-50 rounded-lg p-3">
                      <div className="font-medium text-green-700">{player.name}</div>
                      <div className="text-sm text-gray-600">{player.role}</div>
                      <div className="text-xs text-gray-500 mt-1">{player.details}</div>
                    </div>
                  ))}
                </div>
              </CardContent>
      ...
      // Fallback for ai-analysis without specific data
      return (
        <div className="flex justify-start mb-3">
          <div className="bg-gray-200 rounded-2xl rounded-tl-none px-4 py-2 max-w-[80%]">
            {message.content}
