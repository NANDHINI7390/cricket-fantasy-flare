...
const MatchScorecardAnalysis: React.FC<MatchScorecardAnalysisProps> = ({ matchId }) => {
  ...
  return (
    <div className="container mx-auto px-4 py-8">
      <div className="mb-6">
        <h2 className="text-2xl font-bold text-gray-800">{scorecard.name}</h2>
        <p className="text-gray-600">{scorecard.venue} • {scorecard.date}</p>
        <div className="mt-2">
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              {analysis.bestBatsmen.length > 0 ? (
                ...
                <p className="text-gray-600 mt-1">No batting data available</p>
              )}
            </div>
          </div>
        </Card>
          ...
          <div className="flex items-start">
            ...
            <div>
              ...
              {analysis.bestBowlers.length > 0 ? (
                ...
                <p className="text-gray-600 mt-1">No bowling data available</p>
              )}
            </div>
          </div>
        </Card>
            ...
            {analysis.recommendedTeam.map((player: any, index: number) => (
              <div key={`${player.name}-${index}`} className="flex items-center space-x-2 p-2 rounded-lg bg-gray-50 border">
                <div className={`h-8 w-8 rounded-full flex items-center justify-center text-white font-bold ${
                  ...
                  player.role === "batsman" ? "bg-blue-600" : "bg-green-600"
                }`}>
